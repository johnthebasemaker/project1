const materials = [
    ...
    // Get gallery and search bar elements
    const gallery = document.getElementById("gallery");
    const searchBar = document.getElementById("search-bar");
    
    // Function to display materials in the gallery
    function displayMaterials(filteredMaterials) {
        gallery.innerHTML = ""; // Clear existing gallery content
        filteredMaterials.forEach(material => {
            const card = document.createElement("div");
            card.classList.add("col-6", "col-md-4", "col-lg-3", "mb-4"); // Responsive columns
    ...
    // Search functionality with Fuse.js for approximate matches
    const fuse = new Fuse(materials, {
        keys: ["name", "id"], // Search by name and image ID
        threshold: 0.4        // Allow approximate matches
    });
    
    // Search event listener
    searchBar.addEventListener("input", () => {
        const query = searchBar.value.trim().toLowerCase(); // Get and trim the query input
        if (query === "") {
    displayMaterials(materials); // Show all materials if search query is empty
        } else {
            const results = fuse.search(query); // Fuse.js search
            const filteredMaterials = results.map(result => result.item); // Extract items from search results
            displayMaterials(filteredMaterials); // Display filtered materials
        }
    });
    
    // Initial load - display all materials
